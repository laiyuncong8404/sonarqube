...
public class WorkDirectoriesInitializer {
  ...
  private void mkdirsAllWorkingDirs(DefaultInputModule module) {
    ...
    try {
      org.apache.commons.io.FileUtils.forceMkdir(module.getWorkDir().toFile());
    } catch (Exception e) {
      throw new IllegalStateException("Fail to create working dir: " + module.getWorkDir(), e);
    }
  }

  private void deleteAllRecursivelyExceptLockFile(Path dirToDelete) {
  ...
  private DirectoryStream<Path> list(Path dir) throws IOException {
